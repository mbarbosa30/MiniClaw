...
export function SkillsView() {
  const agentId = useRouter(s => s.currentView.params?.id);
  const { data: skills, isLoading } = useSkills(agentId);
  const toggle = useToggleSkill();

  return (
    <SubScreenLayout title="Skills">
      {isLoading ? <LoadingState /> : !skills?.length ? (
        <EmptyState icon="⚡" title="No skills available" description="Skills will appear here when the API returns them." />
      ) : (
        skills.map(skill => (
      ...
      <div className="bg-white rounded-2xl p-4 border border-primary/10 shadow-sm">
        <div className="flex gap-1.5 mb-4 bg-muted p-1 rounded-xl">
          {(['text', 'url'] as const).map(t => (
            <button
              key={t}
              className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all capitalize flex items-center justify-center gap-2 ${type === t ? 'bg-white text-primary shadow-sm' : 'text-muted-foreground'}`}
              onClick={() => setType(t)}
            >
          ...
          <div className="space-y-2.5">
            {knowledge.map((k: any) => (
              <div key={k.id} className="bg-white rounded-2xl p-4 flex items-start gap-3 border border-black/5 shadow-sm">
                ...
                <div className="flex-1 text-sm break-all leading-relaxed line-clamp-3 text-foreground/80">
                  {k.content}
                </div>
                <button
...
export function SoulView() {
  ...
  return (
    <SubScreenLayout title="Soul Document">
      <div className="bg-muted/30 rounded-2xl p-3 mb-2 border border-primary/10">
        <p className="text-xs text-muted-foreground leading-relaxed">
          The Soul document defines your agent's core identity, tone, and behavior. Changes take effect immediately.
        </p>
      </div>
...
export function MemoriesView() {
  ...
  const handleSaveEdit = (memoryId: string) => {
    updateMemory.mutate({ agentId, id: memoryId, data: { content: editContent } }, {
      onSuccess: () => { setEditingId(null); setEditContent(''); }
    });
  };

  const handlePin = (memory: any) => {
  ...
  return (
    <SubScreenLayout title="Memories">
      {isLoading ? <LoadingState /> : !memories?.length ? (
        <EmptyState icon="🧠" title="No memories yet" description="Your agent will form memories as you chat and interact over time." />
      ) : (
        memories.map((memory: any) => (
          <div key={memory.id} className={`bg-white rounded-2xl p-4 border shadow-sm transition-all ${memory.pinned ? 'border-primary/20 bg-primary/5' : 'border-black/5'}`}>
            {editingId === memory.id ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="flex-1" onClick={() => handleSaveEdit(memory.id)} disabled={updateMemory.isPending}>
                    Save
                  </Button>
...
export function TasksView() {
  const agentId = useRouter(s => s.currentView.params?.id);
  const { data: pendingTasks, isLoading: pendingLoading } = useTasks(agentId, 'pending');
  const { data: allTasks, isLoading: allLoading } = useTasks(agentId, 'all');
  const resolve = useResolveTask();
  const [tab, setTab] = useState<'pending' | 'all'>('pending');

  const tasks = tab === 'pending' ? pendingTasks : allTasks;
  const isLoading = tab === 'pending' ? pendingLoading : allLoading;

  return (
    <SubScreenLayout title="Pending Tasks">
      <div className="flex gap-1 bg-muted p-1 rounded-xl mb-2">
        {(['pending', 'all'] as const).map(t => (
          <button
            key={t}
            className={`flex-1 py-2 text-sm font-semibold rounded-lg capitalize transition-all ${tab === t ? 'bg-white text-primary shadow-sm' : 'text-muted-foreground'}`}
            onClick={() => setTab(t)}
          >
      ...
      {isLoading ? <LoadingState /> : !tasks?.length ? (
        <EmptyState icon="✅" title={tab === 'pending' ? 'No pending tasks' : 'No tasks yet'} description={tab === 'pending' ? "Your agent has no autonomous actions waiting for approval." : "Your agent hasn't started any tasks yet."} />
      ) : (
        tasks.map((task: any) => (
          <div key={task.id} className="bg-white rounded-2xl p-4 border border-black/5 shadow-sm">
            <div className="flex items-start justify-between gap-2 mb-3">
              <p className="text-sm font-semibold flex-1">{task.title || task.description || task.action || 'Task'}</p>
              {task.status && (
            ...
            {(tab === 'pending' || task.status === 'pending') && (
              <div className="flex gap-2">
                <button
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-green-500/10 text-green-700 text-sm font-semibold hover:bg-green-500/20 transition-colors"
                ...
                <button
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-destructive/10 text-destructive text-sm font-semibold hover:bg-destructive/20 transition-colors"
...
function useTelegramStatus(agentId: string) {
  return useQuery({
    queryKey: ['telegram-status', agentId],
    queryFn: () => apiFetch<TelegramStatus>(`/api/selfclaw/v1/hosted-agents/${agentId}/telegram/status`),
    enabled: !!agentId,
  });
}
...
export function TelegramView() {
  ...
  const connect = useMutation({
    mutationFn: () => apiFetch(`/api/selfclaw/v1/hosted-agents/${agentId}/telegram/connect`, {
      method: 'POST',
  ...
  const disconnect = useMutation({
    mutationFn: () => apiFetch(`/api/selfclaw/v1/hosted-agents/${agentId}/telegram/disconnect`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['telegram-status', agentId] })
  });

  const updateSettings = useMutation({
    mutationFn: () => apiFetch(`/api/selfclaw/v1/hosted-agents/${agentId}/telegram/settings`, {
      method: 'PATCH',
      ...
      {isLoading ? <LoadingState /> : (
        <>
          ...
          <div className={`rounded-2xl p-4 flex items-center gap-3 border ${status?.connected ? 'bg-green-50 border-green-200' : 'bg-muted border-border'}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl ${status?.connected ? 'bg-green-100' : 'bg-muted'}`}>
              ✈️
            </div>
            <div className="flex-1">
              <p className="font-semibold text-sm">{status?.connected ? 'Connected' : 'Not Connected'}</p>
              {status?.connected && status.username && (
                <p className="text-xs text-muted-foreground mt-0.5">@{status.username}</p>
              )}
              {!status?.connected && (
                <p className="text-xs text-muted-foreground mt-0.5">Connect a Telegram bot to chat on Telegram</p>
              )}
            </div>
            {status?.connected && (
            ...
            <div className="space-y-3">
              <div className="bg-white rounded-2xl p-4 border border-black/5 shadow-sm space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    placeholder="Welcome message sent to new users..."
                    value={greeting}
                ...
                <Button
                  ...
                  disabled={updateSettings.isPending || !greeting.trim()}
                >
              ...
              <Button
                ...
                className="w-full"
                onClick={() => {
                  if (confirm('Disconnect Telegram bot?')) disconnect.mutate();
                }}
          ...
          {!status?.connected && (
            <div className="bg-secondary/20 rounded-2xl p-4 border border-secondary/30">
              ...
              <ol className="text-xs text-muted-foreground space-y-1.5 list-decimal list-inside leading-relaxed">
                ...
                <li>Send <code className="bg-white/60 px-1 rounded">/newbot</code> and follow the prompts</li>
